...
//    fun insertIntoSaved(vararg articles: Article?)
    ...
    @Query("DELETE from saved_tbl")
    fun deleteAllSavedArticles()
    @Query("DELETE from saved_tbl where id = :id")
    fun deleteSavedArticleById(id:Int)
    @Query("DELETE from saved_tbl where title = :title")
    fun deleteSavedArticleByTitle(title: String)
    @Query("SELECT id from saved_tbl where title = :title")
    fun getSavedArticleIdByTitle(title:String): Int
